<template>
<!-- Bootstrap Modal -->
<div class="modal fade" id="addTailorShopModal" tabindex="-1" aria-labelledby="addTailorShopLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addTailorShopLabel">Add Tailor Shop</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form @submit.prevent="CreateTailorShop">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                    ...
                    <div class="mb-3 p-3 bg-light rounded shadow-sm">
                        <div class="d-flex align-items-center">
                            <img src="/public/assets/images/placeholder/placeholder-image.png" alt="Shop Logo" class="rounded shadow me-3" ref="shopLogoImageDisplay" width="150" height="150" style="object-fit: cover;" />
                            <div class="ms-4">
...
export default {
    setup(props, ctx) {
        ...
        const showModal = () => new Modal(document.getElementById("addTailorShopModal")).show();

        function hideModal() {
            addTailorShopModal.value = false;
        }
        async function CreateTailorShop() {
            try {
                state.errors = {};
                ...
                const response = await tailorShopService.createTailorShop(formData);
                if (response.status !== 200 && response.status !== 201) {
                    throw new Error(`Unexpected response status: ${response.status}`);
                }
                for (const key in state.tailorShop) {
                    state.tailorShop[key] = '';
                }
                // Reset the image preview
                shopLogoImageDisplay.value.src = '/assets/images/placeholder/placeholder-image.png';
                
                ctx.emit('tailorShopAdded', response.data.tailorShop);
                Modal.getInstance(document.getElementById("addTailorShopModal"))?.hide();
                toast.success("Tailor shop added successfully");
        ...
        function removeImage() {
            ...
            shopLogoImageDisplay.value.src = '/assets/images/placeholder/placeholder-image.png'; // Use a valid placeholder image URL
        }
        return {
            ...toRefs(state),
            showModal,
